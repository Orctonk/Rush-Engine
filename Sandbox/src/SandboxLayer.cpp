...
void SandboxLayer::OnAttach()
{
	...
	for (int i = 0; i < 5; i++)
	{
		Entity box = m_Scene.NewEntity("Stacked Box " + std::to_string(i));
		box.AddComponent<MeshRendererComponent>(AssetManager::GetMesh("res/models/cube.obj"));
		auto &bbox = box.AddComponent<BoxCollider>();
		bbox.size = glm::vec3(2.0f, 2.0f, 2.0f);
		box.AddComponent<Rigidbody>(1.0f, 0.8f);
		auto &btc = box.GetComponent<TransformComponent>();
		btc.Translate({0.0f, i * 2.0f - 4, 0.0f});
	}
