...
RootMesh ModelLoader::LoadModel(const Path &filepath) {
    ...
    if (!s_DefaultColorTexture)
        s_DefaultColorTexture = AssetManager::GetTexture("res/textures/white.png");
    if (!s_DefaultNormalTexture)
        s_DefaultNormalTexture = AssetManager::GetTexture("res/textures/blue.png");
...
Shared<Material> ModelLoader::ProcessMaterial(const aiMaterial *material, const aiScene *scene, const std::string &parentMesh) {
    ...
    if (ret != AI_FAILURE)
        mat->diffuseTexture = AssetManager::GetTexture(s_CurDirectory.AppendFile(str.C_Str()));
    else
        mat->diffuseTexture = s_DefaultColorTexture;
    ret = material->GetTexture(aiTextureType_SPECULAR, 0, &str);
    if (ret != AI_FAILURE)
        mat->specularTexture = AssetManager::GetTexture(s_CurDirectory.AppendFile(str.C_Str()));
    else
        mat->specularTexture = s_DefaultColorTexture;
    ret = material->GetTexture(aiTextureType_NORMALS, 0, &str);
    if (ret != AI_FAILURE)
        mat->normalTexture = AssetManager::GetTexture(s_CurDirectory.AppendFile(str.C_Str()));
    else
        mat->normalTexture = s_DefaultNormalTexture;
