...
static void APIENTRY glDebugOutput(GLenum source,
    ...
    switch (source) {
        case GL_DEBUG_SOURCE_API:
            sourcestr = std::string("API");
            break;
        case GL_DEBUG_SOURCE_WINDOW_SYSTEM:
            sourcestr = std::string("Window System");
            break;
        case GL_DEBUG_SOURCE_SHADER_COMPILER:
            sourcestr = std::string("Shader Compiler");
            break;
        case GL_DEBUG_SOURCE_THIRD_PARTY:
            sourcestr = std::string("Third Party");
            break;
        case GL_DEBUG_SOURCE_APPLICATION:
            sourcestr = std::string("Application");
            break;
        case GL_DEBUG_SOURCE_OTHER:
            sourcestr = std::string("Other");
            break;
    }

    switch (type) {
        case GL_DEBUG_TYPE_ERROR:
            messagestr += std::string(" Type: Error");
            break;
        case GL_DEBUG_TYPE_DEPRECATED_BEHAVIOR:
            messagestr += std::string(" Type: Deprecated Behaviour");
            break;
        case GL_DEBUG_TYPE_UNDEFINED_BEHAVIOR:
            messagestr += std::string(" Type: Undefined Behaviour");
            break;
        case GL_DEBUG_TYPE_PORTABILITY:
            messagestr += std::string(" Type: Portability");
            break;
        case GL_DEBUG_TYPE_PERFORMANCE:
            messagestr += std::string(" Type: Performance");
            break;
        case GL_DEBUG_TYPE_MARKER:
            messagestr += std::string(" Type: Marker");
            break;
        case GL_DEBUG_TYPE_PUSH_GROUP:
            messagestr += std::string(" Type: Push Group");
            break;
        case GL_DEBUG_TYPE_POP_GROUP:
            messagestr += std::string(" Type: Pop Group");
            break;
        case GL_DEBUG_TYPE_OTHER:
            messagestr += std::string(" Type: Other");
            break;
    }

    switch (severity) {
        case GL_DEBUG_SEVERITY_HIGH:
            Logger::Error(messagestr, sourcestr);
            break;
        case GL_DEBUG_SEVERITY_MEDIUM:
            Logger::Warning(messagestr, sourcestr);
            break;
        case GL_DEBUG_SEVERITY_LOW:
            Logger::Info(messagestr, sourcestr);
            break;
        case GL_DEBUG_SEVERITY_NOTIFICATION:
            Logger::Trace(messagestr, sourcestr);
            break;
    }
}
...
GLFWWindow::GLFWWindow(const WindowProperties &properties) : AbstractWindow(properties) {
    RUSH_PROFILE_FUNCTION();
    if (s_WindowCount == 0) {
